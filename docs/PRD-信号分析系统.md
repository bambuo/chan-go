...
一个**给自动化交易机器人消费的缠论信号引擎**：消费外部数据适配层写入的 K 线流，实时计算真递归多级别缠论结构，对外提供买卖点信号的 REST 查询。
...
> ⚠️ 与输入协议的对称设计：输入用 `chan:klines:{symbol}`（XREADGROUP），输出用 `chan:signals:{symbol}`（XADD），输入输出共用同一 Redis 实例。这种对称设计让回测场景只需向输入 stream 灌历史 K 线，输出 stream 即自动产历史信号。
