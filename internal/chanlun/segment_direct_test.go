...
// ====== confirmSegmentBreak 直接测试 ======

func TestConfirmSegmentBreak(t *testing.T) {
	bi0 := &stroke{Index: 0, Direction: types.DirectionUp, StartPrice: 10, EndPrice: 20, Confirmed: true}
	bi1 := &stroke{Index: 1, Direction: types.DirectionUp, StartPrice: 20, EndPrice: 30, Confirmed: true}
	bi2 := &stroke{Index: 2, Direction: types.DirectionDown, StartPrice: 30, EndPrice: 25, Confirmed: true}

	currSeg := &segment{
		index:     0,
		direction: types.DirectionUp,
		strokes:   []*stroke{bi0, bi1, bi2},
	}
	...
	state.confirmSegmentBreak(pendingSeg)
	...
	completed := state.segments[0]
	if !completed.confirmed {
		t.Error("已完成的线段 confirmed 应为 true")
	}
	if len(completed.strokes) != 2 {
		t.Errorf("已完成线段 strokes 期望 2 (bi0, bi1), 实际 %d", len(completed.strokes))
	}
	if completed.startPrice != 10 {
		t.Errorf("startPrice 期望 10, 实际 %f", completed.startPrice)
	}
	if completed.endPrice != 30 {
		t.Errorf("endPrice 期望 30, 实际 %f", completed.endPrice)
	}

	// 验证 pendingSeg 成为当前线段
	if state.current != pendingSeg {
		t.Error("pendingSeg 应成为当前线段")
	}
	if state.pending != nil {
		t.Error("pending 应被清空")
	}
}

func TestConfirmSegmentBreak_NoSplit(t *testing.T) {
	// 测试 breakIdx 找不到的情况（不应发生，但不 panic）
	bi0 := &stroke{Index: 0, Direction: types.DirectionUp, Confirmed: true}
	bi1 := &stroke{Index: 1, Direction: types.DirectionDown, Confirmed: true}

	currSeg := &segment{
		strokes: []*stroke{bi0},
	}
	pendingSeg := &segment{
		strokes: []*stroke{bi1}, // bi1 不在 currSeg 中
	}
